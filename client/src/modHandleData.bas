...
Public Sub InitMessages()
    ...
    HandleDataSub(SCreateChar) = GetAddress(AddressOf HandleCreateChar)
    ...
    HandleDataSub(SSendSkills) = GetAddress(AddressOf HandleSendSkills)
    HandleDataSub(SSendSkill) = GetAddress(AddressOf HandleSendSkill)
    HandleDataSub(SProgressConversation) = GetAddress(AddressOf HandleProgressConversation)
    HandleDataSub(SSendUpdateShopStock) = GetAddress(AddressOf HandleUpdateShopStock)
    HandleDataSub(SUpdateSpecial) = GetAddress(AddressOf HandleUpdateSpecial)
    HandleDataSub(SUpdatePlayerSprite) = GetAddress(AddressOf HandleUpdatePlayerSprite)
    HandleDataSub(SUpdateSummoningCreature) = GetAddress(AddressOf HandleUpdateSummoningCreature)
    HandleDataSub(SUpdateClanList) = GetAddress(AddressOf HandleUpdateClanList)
    ' Error handler
    Exit Sub
errorhandler:
...
Dim i As Long
    ...
    For i = 1 To Equipment.Equipment_Count - 1
        ...
        Player(MyIndex).Equipment(i).Value = Buffer.ReadLong
    Next
...
Dim i As Long
    ...
    For i = 1 To Equipment.Equipment_Count - 1
        ...
        Player(playerNum).Equipment(i).Value = Buffer.ReadLong
    Next
...
Dim Buffer As clsBuffer
    ...
    Player(i).SpecialAttack = Buffer.ReadLong
...
Dim Buffer As clsBuffer
    ...
    If FileExist(MAP_PATH & "map" & x & MAP_EXT, False) Then
        ...
        If Map.Revision = y And x <= MAX_MAPS - MAX_PLAYERS Then
...
Dim Buffer As clsBuffer
    ...
    For i = 1 To MAX_MAP_NPCS
        With MapNpc(i)
            ...
            .y = Buffer.ReadLong
            .Dir = Buffer.ReadLong
            .Vital(Vitals.HitPoints) = Buffer.ReadLong
        End With
    Next
...
Dim color As Byte
    ...
    If Msg = "You do not have a target." Or Msg = "Target not in range." Then
        SpellBuffer = 0
        SpellBufferTimer = 0
    End If
...
Dim saycolour As Long
    ...
    Call AddText(Header & Name & ": " & message, colour, Header)
    
    Exit Sub
    
    frmMain.txtGlobalChat.SelStart = Len(frmMain.txtGlobalChat.Text)
    frmMain.txtGlobalChat.SelColor = colour
    frmMain.txtGlobalChat.SelText = vbNewLine & Header & Name & ": "
    frmMain.txtGlobalChat.SelColor = saycolour
    frmMain.txtGlobalChat.SelText = message
...
Dim i As Long
    ...
    For i = 1 To MAX_TRADES
        Shop(ShopNum).TradeItem(i).Stock = Buffer.ReadLong
    Next
...
Dim i As Long, t As Long
    ...
    If BankTab < 1 Or BankTab > MAX_BANK_TABS Then BankTab = 1
    BltBank
    
    frmMain.picBank.SetFocus
...
Dim Buffer As clsBuffer
    ...
    ProjectileIndex = Buffer.ReadLong
    
    ' populate the values
    With MapProjectile.Projectile(ProjectileIndex)
...
Public Sub HandleCreateChar(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
...
Public Sub HandleSendSkills(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
Dim i As Long
    ...
    For i = 1 To Skills.Skill_Count - 1
        Player(MyIndex).Skill(i).MaxLevel = Buffer.ReadLong
        Player(MyIndex).Skill(i).Level = Buffer.ReadLong
        Player(MyIndex).Skill(i).XP = Buffer.ReadLong
    Next
    
    frmMain.lblHP.Caption = Player(MyIndex).Skill(Skills.HitPoints).Level & "/" & Player(MyIndex).Skill(Skills.HitPoints).MaxLevel
    ' hp bar
    frmMain.imgHPBar.width = ((Player(MyIndex).Skill(Skills.HitPoints).Level / HPBar_Width) / (Player(MyIndex).Skill(Skills.HitPoints).MaxLevel / HPBar_Width)) * HPBar_Width
    frmMain.lblPR.Caption = Player(MyIndex).Skill(Skills.Prayer).Level & "/" & Player(MyIndex).Skill(Skills.Prayer).MaxLevel
    ' hp bar
    frmMain.imgPRBar.width = ((Player(MyIndex).Skill(Skills.Prayer).Level / HPBar_Width) / (Player(MyIndex).Skill(Skills.Prayer).MaxLevel / HPBar_Width)) * HPBar_Width
    frmMain.lblSUM.Caption = Player(MyIndex).Skill(Skills.Summoning).Level & "/" & Player(MyIndex).Skill(Skills.Summoning).MaxLevel
    ' hp bar
    frmMain.imgSUMBar.width = ((Player(MyIndex).Skill(Skills.Summoning).Level / HPBar_Width) / (Player(MyIndex).Skill(Skills.Summoning).MaxLevel / HPBar_Width)) * HPBar_Width

End Sub

Public Sub HandleSendSkill(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
Dim Skill As Long
    ...
    Player(MyIndex).Skill(Skill).MaxLevel = Buffer.ReadLong
    Player(MyIndex).Skill(Skill).Level = Buffer.ReadLong
    Player(MyIndex).Skill(Skill).XP = Buffer.ReadLong
    
    Select Case Skill
        Case Skills.HitPoints
            frmMain.lblHP.Caption = Player(MyIndex).Skill(Skill).Level & "/" & Player(MyIndex).Skill(Skill).MaxLevel
            ' hp bar
            frmMain.imgHPBar.width = ((Player(MyIndex).Skill(Skill).Level / HPBar_Width) / (Player(MyIndex).Skill(Skill).MaxLevel / HPBar_Width)) * HPBar_Width
        Case Skills.Prayer
            frmMain.lblPR.Caption = Player(MyIndex).Skill(Skill).Level & "/" & Player(MyIndex).Skill(Skill).MaxLevel
            ' hp bar
            frmMain.imgPRBar.width = ((Player(MyIndex).Skill(Skill).Level / HPBar_Width) / (Player(MyIndex).Skill(Skill).MaxLevel / HPBar_Width)) * HPBar_Width
        Case Skills.Summoning
            frmMain.lblSUM.Caption = Player(MyIndex).Skill(Skill).Level & "/" & Player(MyIndex).Skill(Skill).MaxLevel
            ' hp bar
            frmMain.imgSUMBar.width = ((Player(MyIndex).Skill(Skill).Level / HPBar_Width) / (Player(MyIndex).Skill(Skill).MaxLevel / HPBar_Width)) * HPBar_Width
    End Select
End Sub

Public Sub HandleProgressConversation(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
Dim NpcNum As Long
    ...
    ConvState = Buffer.ReadLong
    NpcNum = Buffer.ReadLong
    
    ' Is it a valid progression of the conversation? -1 is terminating the conversation.
    If ConvState >= 0 Then
        ' This is the start of the conversation
        If frmMain.picConv.Visible = False Then
            frmMain.picConv.Visible = True
            frmMain.picConvFace.Picture = Nothing
        End If
        
        If NpcNum <> ConvNpc Then
            ConvNpc = NpcNum
            If FileExist(App.Path & GFX_PATH & NpcNum & GFX_EXT, True) Then
                frmMain.picConvFace.Picture = LoadPicture(App.Path + GFX_PATH + NpcNum + GFX_EXT)
            End If
        End If
        
        frmMain.lblConvNPCName.Caption = Trim$(Npc(NpcNum).Name)
        frmMain.lblConvMessage.Caption = Buffer.ReadString
        frmMain.lblConvResp(1).Caption = Buffer.ReadString
        frmMain.lblConvResp(2).Caption = Buffer.ReadString
        frmMain.lblConvResp(3).Caption = Buffer.ReadString
        frmMain.lblConvResp(4).Caption = Buffer.ReadString
    Else
        InConv = 0
        ConvNpc = 0
...
Public Sub HandleUpdateShopStock(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
Dim ShopNum As Long
Dim Slot As Long
Dim Amount As Long
    ...
    If ShopNum <> InShop Then MsgBox ("Got a wrong ShopUpdate packet")
    Shop(ShopNum).TradeItem(Slot).Stock = Amount
...
Public Sub HandleUpdateSpecial(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer

    Set Buffer = New clsBuffer
    Buffer.WriteBytes Data()
    
    Player(MyIndex).SpecialAttack = Buffer.ReadLong
    
    If frmMain.picCombat.Visible Then
...
Public Sub HandleUpdatePlayerSprite(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer

    Set Buffer = New clsBuffer
    Buffer.WriteBytes Data()
    Index = Buffer.ReadLong
    Player(Index).Sprite = Buffer.ReadLong
    Set Buffer = Nothing
End Sub

Public Sub HandleUpdateSummoningCreature(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer

    Set Buffer = New clsBuffer
    Buffer.WriteBytes Data()
    Player(MyIndex).SummoningCreature.NpcNum = Buffer.ReadLong
    Player(MyIndex).SummoningCreature.Health = Buffer.ReadLong
    Player(MyIndex).SummoningCreature.Special = Buffer.ReadLong
    Set Buffer = Nothing
End Sub

Public Sub HandleUpdateClanList(ByVal Index As Long, ByRef Data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
Dim Buffer As clsBuffer
Dim i As Long
Dim Value As Long
    ...
    If Value = -255 Then
        Clan.LootShare = 0
        For i = 1 To MAX_PLAYERS
            Clan.Member(i).playerIndex = 0
            Clan.Member(i).Rank = vbNullString
        Next
    Else
        Clan.LootShare = Value
        For i = 1 To MAX_PLAYERS
            Clan.Member(i).playerIndex = Buffer.ReadLong
            Clan.Member(i).Rank = Buffer.ReadString
        Next
    End If
