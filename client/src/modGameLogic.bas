    ...
    Do While InGame
        ...
        If tmr25 < Tick Then
            ...
            If NumSpellIcons > 0 Then
                For i = 1 To MAX_PLAYER_SPELLS
                    If PlayerSpells(i) > 0 Then
                        If SpellCD(i) > 0 Then
                            If SpellCD(i) + (Spell(PlayerSpells(i)).CooldownTime * 1000) < Tick Then
                                SpellCD(i) = 0
                                BltPlayerSpells
                                BltHotbar
                            End If
            ...
            If SpellBuffer > 0 Then
                If SpellBufferTimer + (Spell(PlayerSpells(SpellBuffer)).CastingTime * 1000) < Tick Then
                    SpellBuffer = 0
                    SpellBufferTimer = 0
                End If
            End If
    ...
    If ControlDown Then
        ...
        If Player(MyIndex).AttackTimer + AttackSpeed < GetTickCount Then
            If Player(MyIndex).Attacking = 0 Then
                ...
                If Special > 0 Then
                    Special = 0
                    frmMain.lblSpec.ForeColor = &H808080
                End If
            End If
        End If
    End If
...
Dim i As Long
    ...
    If Map.Tile(x, y).Type = TILE_TYPE_BLOCKED Or Map.Tile(x, y).Type = TILE_TYPE_OBJECT Then
        CheckDirection = True
        Exit Function
    End If
...
Public Sub CastSpell(ByVal spellslot As Long, Optional ByVal InvNum As Long = 0)
Dim Buffer As clsBuffer
...
Dim i As Long
    ...
    With frmMain
        ...
        .lblSpellDesc.Caption = Trim$(Spell(spellnum).Description)
...
Dim Name As String
    ...
    With frmMain
        ...
        .lblItemDesc.Caption = Trim$(Item(ItemNum).Description)
...
Public Sub OpenShop(ByVal ShopNum As Long)
    ...
    If Shop(InShop).InterfaceNum > 0 Then
        If FileExist("data files\graphics\gui\main\shops\" & Shop(InShop).InterfaceNum & ".jpg") And FileExist("data files\graphics\gui\main\shops\" & Shop(InShop).InterfaceNum & "top.jpg") Then
            frmMain.picShop.Picture = LoadPicture(App.Path & "\data files\graphics\gui\main\shops\" & Shop(InShop).InterfaceNum & ".jpg")
            frmMain.picShopItems.Picture = LoadPicture(App.Path & "\data files\graphics\gui\main\shops\" & Shop(InShop).InterfaceNum & "top.jpg")
        Else
...
Public Function GetBankItemNum(ByVal bankslot As Long) As Long
    ...
    GetBankItemNum = Bank.BankTab(BankTab).Item(bankslot).num
...
Public Sub SetBankItemNum(ByVal bankslot As Long, ByVal ItemNum As Long)
    ...
    Bank.BankTab(BankTab).Item(bankslot).num = ItemNum
...
Public Function GetBankItemValue(ByVal bankslot As Long) As Long
    ...
    GetBankItemValue = Bank.BankTab(BankTab).Item(bankslot).Value
...
Public Sub SetBankItemValue(ByVal bankslot As Long, ByVal ItemValue As Long)
    ...
    Bank.BankTab(BankTab).Item(bankslot).Value = ItemValue
