...
Begin VB.Form frmMain 
   ...
   Begin VB.PictureBox picCombat 
      ...
      Begin VB.Label lblSpec 
         ...
         Width           =   2250
      End
      Begin VB.Image imgSpec 
         Height          =   135
         Left            =   840
         Top             =   3720
         Width           =   975
      End
      Begin VB.Image imgSpec_Void 
         Height          =   135
         Left            =   720
         Top             =   3600
         Width           =   975
   ...
   Begin VB.PictureBox picCharacter 
      ...
      End
   End
   Begin RichTextLib.RichTextBox txtGameChat 
      Height          =   1800
      Left            =   7560
      TabIndex        =   70
      Top             =   7320
      ...
      EndProperty
   End
   Begin RichTextLib.RichTextBox txtPrivateChat 
      Height          =   1800
      Left            =   7680
      TabIndex        =   71
      Top             =   6840
      ...
      EndProperty
   End
   Begin RichTextLib.RichTextBox txtClanChat 
      Height          =   1800
      Left            =   7800
      TabIndex        =   72
      Top             =   6600
   ...
   Begin VB.Label lblHP 
      ...
      Width           =   1845
   End
   Begin VB.Image imgSUMBar 
      Height          =   240
      Left            =   7770
      Top             =   1080
      Width           =   3615
   End
   Begin VB.Image imgPRBar 
      Height          =   240
      Left            =   7770
      Top             =   750
      Width           =   3615
...
Private Sub Form_Load()
    ...
    ' move GUI
    picAdmin.Left = 544
    picCurrency.Left = txtGlobalChat.Left
    picCurrency.top = txtGlobalChat.top
    picDialogue.top = txtGlobalChat.top
    picDialogue.Left = txtGlobalChat.Left
...
Dim i As Long
    ...
    If picShopItem.Visible = True Then picShopItem.Visible = False
    imgSpellDown.Visible = False
    imgSpellUp.Visible = False
    
    SpellSlotNum = 0
    
    Select Case Index
        Case Tabs.TabCombat
            If Not picCombat.Visible Then
                picCombat.Visible = True
                ...
                picClan.Visible = False
                picSkills.Visible = False
                BltCombat
            End If
        Case Tabs.TabQuests
            If Not picQuests.Visible Then
                picCombat.Visible = False
                picSkills.Visible = False
        ...
        Case Tabs.TabInventory
            ...
            End If
        Case Tabs.TabSpells
            imgSpellUp.Visible = True
            imgSpellDown.Visible = True
            picCombat.Visible = False
...
Private Sub imgJoin_Click()
Dim ClanName As String * NAME_LENGTH

    ClanName = InputBox("Enter the name of an online player.", "Join a Clan")
    Call SendRequestJoinClan(Trim$(ClanName))

End Sub

Private Sub imgLeave_Click()
    SendLeaveClan
End Sub

Private Sub imgSpellDown_MouseDown(Button As Integer, Shift As Integer, x As Single, y As Single)
    If SpellListColumn = 3 Then Exit Sub
    
    SpellListColumn = SpellListColumn + 1
    
    BltPlayerSpells
End Sub

Private Sub imgSpellUp_MouseDown(Button As Integer, Shift As Integer, x As Single, y As Single)
    If SpellListColumn = 0 Then Exit Sub
...
Private Sub Label1_Click()
    ...
    Dim Amount As Long
    stringAmount = InputBox("Please select a custom multiplier.")
    If IsNumeric(stringAmount) Then
        If stringAmount > 500 Then
            ShopMultiplier = 500
        ElseIf stringAmount < 1 Then
            ShopMultiplier = 1
        Else
            ShopMultiplier = stringAmount
        End If
    Else
        ShopMultiplier = 0
    End If
    
    If ShopAction = 1 Then lblBuySell.Caption = "Buy x" & ShopMultiplier
    If ShopAction = 2 Then lblBuySell.Caption = "Sell x" & ShopMultiplier
End Sub


Private Sub lblBuySell_Click()

    Select Case ShopAction
        Case 1
            BuyItem ShopItemSlot, ShopMultiplier
        Case 2
            SellItem ShopItemSlot, ShopMultiplier
    End Select
        
End Sub

Private Sub lblConvResp_Click(Index As Integer)
    Call SendProgressConversation(MyIndex, Index)
End Sub
...
Private Sub lblSpec_Click()

    If Special > 0 Then
        Special = 0
        lblSpec.ForeColor = &H808080
    Else
        Special = 1
        lblSpec.ForeColor = &HFFFFFF
    End If

End Sub

Private Sub optClan_Click()
txtGlobalChat.Visible = False
txtGameChat.Visible = False
txtPrivateChat.Visible = False
txtClanChat.Visible = True
End Sub

Private Sub optGame_Click()
txtGlobalChat.Visible = False
txtGameChat.Visible = True
txtPrivateChat.Visible = False
txtClanChat.Visible = False
End Sub

Private Sub optGlobal_Click()
txtGlobalChat.Visible = True
txtGameChat.Visible = False
txtPrivateChat.Visible = False
txtClanChat.Visible = False
End Sub
...
Private Sub picBank_KeyDown(KeyCode As Integer, Shift As Integer)
    ' Handle when in the bank tab.
    If InBank Then
        If KeyCode = vbKey1 Then BankMultiplier = 1
        If KeyCode = vbKey2 Then BankMultiplier = 2
        If KeyCode = vbKey3 Then BankMultiplier = 3
        If KeyCode = vbKey4 Then BankMultiplier = 4
        If KeyCode = vbKey5 Then BankMultiplier = 5
        If KeyCode = vbKey6 Then BankMultiplier = 6
        If KeyCode = vbKey7 Then BankMultiplier = 7
        If KeyCode = vbKey8 Then BankMultiplier = 8
        If KeyCode = vbKey9 Then BankMultiplier = 9
        ...
        If KeyCode = vbKey0 Then
            ...
            stringAmount = InputBox("Please select a custom multiplier")
            If IsNumeric(stringAmount) Then
                If stringAmount > 2147483647 Or stringAmount < 1 Then
                    BankMultiplier = 0
                Else
                    BankMultiplier = stringAmount
                End If
            Else
                BankMultiplier = 0
            End If
        ...
        If BankMultiplier = 0 Then
            frmMain.lblMultiplier.Caption = "All"
        Else
            frmMain.lblMultiplier.Caption = "x" & BankMultiplier
        End If
    End If
End Sub
...
Private Sub picInventory_KeyDown(KeyCode As Integer, Shift As Integer)
    ' Handle when in the bank tab.
    If InBank Then
        If KeyCode = vbKey1 Then BankMultiplier = 1
        If KeyCode = vbKey2 Then BankMultiplier = 2
...
Private Sub HandleShopItem(Optional ByVal invslot As Long = 0, Optional ByVal ShopSlot As Long = 0)
Dim ItemNum As Long
    
    If ShopMultiplier > 500 Then ShopMultiplier = 500
    If ShopMultiplier < 1 Then ShopMultiplier = 1

    If invslot > 0 Then
        ItemNum = PlayerInv(invslot).num
        ShopItemSlot = invslot
    ElseIf ShopSlot > 0 Then
        ItemNum = Shop(InShop).TradeItem(ShopSlot).Item
        ShopItemSlot = ShopSlot
    End If
    
    If ShopSlot > 0 Or invslot > 0 Then
        picShopItem.Visible = True
        lblInfo.Caption = Trim$(Item(ItemNum).Name) + ": " + Trim$(Item(ItemNum).Description)
        ...
        If ShopAction = 1 Then
            ...
            With Shop(InShop).TradeItem(ShopSlot)
                For i = 1 To MAX_SHOP_ITEM_COSTS
                    If .CostItem(i) > 0 Then
                        If Cost <> vbNullString Then Cost = Cost & ", "
                        Cost = Cost & .CostValue(i) & "x " & Trim$(Item(.CostItem(i)).Name & " ")
                    End If
                Next
                If Cost = vbNullString Then Cost = "for free"
                lblCost.Caption = "For " & Cost
                Cost = vbNullString
                
                For i = 1 To Skills.Skill_Count - 1
                    If Player(MyIndex).Skill(i).Level < Item(.Item).SkillMakeReq(i) Then
                        If Cost <> vbNullString Then Cost = Cost & ", "
                        Cost = Cost & Item(.Item).SkillMakeReq(i) & " " & GetSkillName(i) & " "
                    End If
                Next
                
                If Cost <> vbNullString Then
                    lblSkillReq.Caption = "Requires: " & Cost
                Else
                    lblSkillReq.Caption = ""
                End If
            End With
            
            lblBuySell.Caption = "Buy x" & ShopMultiplier
            
        End If
        
        If ShopAction = 2 Then
            lblBuySell.Caption = "Sell x" & ShopMultiplier
            lblCost.Caption = "Sell for " & (Item(ItemNum).MonetaryValue * Shop(InShop).BuyRate / 100) & " " & Trim$(Item(Shop(InShop).ShopCurrency).Name) & " each"
            lblSkillReq.Caption = vbNullString
        End If
        
        BltPicShopItem (ItemNum)
    End If
    ...
    If Button = 1 Then ' left click
        If spellnum <> 0 Then
            Select Case spellnum
                Case 5, 10, 13, 20, 23, 28, 29, 34, 43
                    Call imgButton_Click(4)
                    SpellSlotNum = spellnum
                    Exit Sub
            End Select
...
Private Sub txtClanChat_GotFocus()
SetFocusOnChat
End Sub

Private Sub txtGameChat_GotFocus()
SetFocusOnChat
End Sub

Private Sub txtGlobalChat_GotFocus()
SetFocusOnChat
End Sub


Private Sub txtGlobalChat_MouseDown(Button As Integer, Shift As Integer, x As Single, y As Single)
SetFocusOnChat
End Sub
...
Private Sub picInventory_DblClick()
    ...
    If InvNum <> 0 Then
        ...
        If InShop > 0 Then
            ShopAction = 2
            Call HandleShopItem(InvNum)
            Exit Sub
        End If
        
        ' in bank?
        If InBank Then
            Call DepositItem(InvNum, Multiplier)
            Exit Sub
        End If
        ...
        If Item(GetPlayerInvItemNum(MyIndex, InvNum)).ItemType = ITEM_TYPE_NONE Then Exit Sub
...
Private Sub picInventory_MouseDown(Button As Integer, Shift As Integer, x As Single, y As Single)
    ...
    If Button = 1 Then
        If InvNum <> 0 Then
            ...
            If SpellSlotNum > 0 Then
                Call CastSpell(SpellSlotNum, InvNum)
                SpellSlotNum = 0
                Call imgButton_Click(2)
                Exit Sub
            End If
    ...
    ElseIf Button = 2 Then
        If Not InBank And Not InShop And Not InTrade > 0 Then
            If InvNum <> 0 Then
                Call SendDropItem(InvNum, GetPlayerInvItemValue(MyIndex, InvNum))
            End If
        End If
    End If
...
Dim bankNum As Long
    ...
    If bankNum <> 0 Then
         ...
         WithdrawItem bankNum, BankMultiplier
    End If
...
' TODO : Add sub to change bankslots client side first so there's no delay in switching
    If DragBankSlotNum > 0 Then
        ...
        If IsTabNum(x, y) > 0 Then
            SendMoveItemToNewBankTab IsTabNum(x, y), BankTab, DragBankSlotNum
        End If
    End If
...
Dim x2 As Long, y2 As Long
    ...
    Else
        ...
        If bankNum <> 0 Then
            ...
            x2 = x + picBank.Left + 1
            y2 = y + picBank.top + 1
            UpdateDescWindow Bank.BankTab(BankTab).Item(bankNum).num, x2, y2
            Exit Sub
        End If
    End If
...
Private Sub txtPrivateChat_GotFocus()
SetFocusOnChat
End Sub
